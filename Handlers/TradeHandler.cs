...
namespace BscTokenSniper.Handlers
{
	public class TradeHandler : IDisposable
	{
		...
		// TODO: get already owned tokens
		// private async Task<List<string>> GetOwnedTokens(string address)
		// {
		...
		// 	var pancakeFactoryContract = _bscWeb3.Eth.GetContract(_pancakeFactoryAbi, _sniperConfig.PancakeFactoryAddress);
		// 	var getPairFunction = pancakeFactoryContract.GetFunction("getPair");
		// 	var wbnbAddress = "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"; // WBNB token address on BSC
		...
		// 		var pairAddress = await getPairFunction.CallAsync<string>(tx.ContractAddress, wbnbAddress);
